...
export default function App() {
  ...
  const handleResetDatabase = async () => {
    if (!window.confirm(lang === 'ar' ? 'هل أنت تأكد من إعادة ضبط قاعدة البيانات ومسح المهمات المؤقتة؟' : 'Are you sure you want to reset all test data?')) return;
    try {
      setAllRequests([]);
      setIncomingBids([]);
      setChatMessages([]);
      localStorage.removeItem('systro_rescue_requests');
      triggerToast(lang === 'ar' ? 'تمت إعادة ضبط قاعدة البيانات والمحاكاة بنجاح!' : 'Database & Simulator reset successfully!', 'success');
    } catch (err) {
      console.error(err);
      triggerToast(lang === 'ar' ? 'حدث خطأ أثناء إعادة الضبط' : 'Error resetting database', 'error');
    }
  };

  const handleAddOrUpdateService = async () => {
    if (!srvId.trim() || !srvName.trim() || !srvArName.trim()) {
      triggerToast(lang === 'ar' ? 'يرجى ملء جميع الحقول المطلوبة' : 'Please fill all required fields', 'warning');
      return;
    }
    try {
      const srvData = {
        id: srvId.trim().toLowerCase().replace(/\s+/g, '_'),
        name: srvName.trim(),
      ...
      await setDoc(doc(db, "services", srvData.id), srvData, { merge: true });
      triggerToast(lang === 'ar' ? 'تم حفظ بيانات الخدمة بنجاح!' : 'Service saved successfully!', 'success');
      ...
      console.error(err);
      triggerToast(lang === 'ar' ? 'حدث خطأ أثناء حفظ الخدمة' : 'Error saving service', 'error');
    }
  };
  ...
  const handleEditService = (srv: any) => {
    ...
    setSrvId(srv.id || '');
    setSrvName(srv.name || '');
    setSrvArName(srv.arName || '');
    setSrvDesc(srv.description || '');
    setSrvArDesc(srv.arDescription || '');
    setSrvBasePrice(srv.basePrice || 100);
    setSrvIcon(srv.icon || 'wrench');
    setShowAddServiceForm(true);
  };

  const handleDeleteService = async (id: string) => {
    if (!window.confirm(lang === 'ar' ? 'هل أنت تأكد من حذف هذه الخدمة؟' : 'Are you sure you want to delete this service?')) return;
    try {
      await deleteDoc(doc(db, "services", id));
      triggerToast(lang === 'ar' ? 'تم حذف الخدمة بنجاح!' : 'Service deleted successfully!', 'success');
    } catch (err) {
      console.error(err);
      triggerToast(lang === 'ar' ? 'حدث خطأ أثناء الحذف' : 'Error deleting service', 'error');
    }
  };
  ...
  const [notifyWhatsapp, setNotifyWhatsapp] = useState<boolean>(() => {
    return sessionStorage.getItem('systro_notify_whatsapp') === 'true';
  });
  const [notifyEmail, setNotifyEmail] = useState<boolean>(() => {
    return sessionStorage.getItem('systro_notify_email') === 'true';
  });
  ...
  const [activeTechDoc, setActiveTechDoc] = useState<any>(() => {
    const email = (sessionStorage.getItem('systro_user_email') || '').trim().toLowerCase();
    const savedOnline = email ? sessionStorage.getItem(`systro_tech_is_online_${email}`) : sessionStorage.getItem('systro_tech_is_online');
    const savedRole = sessionStorage.getItem('systro_user_role');
    const isOnline = savedOnline !== null ? (savedOnline === 'true') : (savedRole === 'technician');
    return {
      isOnline,
      isAvailable: isOnline,
      name: sessionStorage.getItem('systro_user_name') || 'فني مسجل'
    };
  });
  ...
  const handleToggleTechMode = useCallback(async () => {
    const cleanEmail = (loggedInUserEmail || sessionStorage.getItem('systro_user_email') || '').trim().toLowerCase();
    const savedOnline = cleanEmail ? sessionStorage.getItem(`systro_tech_is_online_${cleanEmail}`) : sessionStorage.getItem('systro_tech_is_online');
    ...
    if (cleanEmail) {
      sessionStorage.setItem(`systro_tech_is_online_${cleanEmail}`, String(nextOnline));
    } else {
      sessionStorage.setItem('systro_tech_is_online', String(nextOnline));
    }
    ...
    triggerToast(
      lang === 'ar'
        ? (nextOnline 
            ? '🟢 تم تفعيل وضع الفني المسجل لحسابك الخاص بنجاح!' 
            : '🔴 تم إيقاف وضع الفني: وضعك غير متاح بالشبكة الآن.')
        : (nextOnline 
            ? '🟢 Tech Mode Active for your logged-in account.' 
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn || !cleanEmail) {
      const currentRole = userRole || sessionStorage.getItem('systro_user_role');
    ...
    const unsub = onSnapshot(docRef, async (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (cleanEmail) {
          sessionStorage.setItem(`systro_tech_is_online_${cleanEmail}`, String(effectiveOnline));
        }
        ...
        const isTechRole = userRole === 'technician' || sessionStorage.getItem('systro_user_role') === 'technician';
    ...
    return () => unsub();
  }, [isLoggedIn, loggedInUserEmail, userRole, loggedInUserName, lang, phoneNumber, providerPhone, userAvatar, providerAvatar, providerVehicle, providerPlate, notifyEmail, notifyWhatsapp]);
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        ...
        const initialServices = [
          { id: 'fuel', name: 'توصيل وتعبئة الوقود', arName: 'توصيل وتعبئة الوقود', enName: 'Fuel Delivery & Refueling', description: 'نفذ منك البنزين أو الديزل على الطريق؟ سنرسل إليك أقرب فني مع كمية الوقود الكافية لإنقاذ رحلتك.', arDescription: 'نفذ منك البنزين أو الديزل على الطريق؟ سنرسل إليك أقرب فني مع كمية الوقود الكافية لإنقاذ رحلتك.', icon: 'fuel', basePrice: 100 },
          { id: 'locksmith', name: 'فتح الأقفال الطارئة', arName: 'فتح الأقفال الطارئة', enName: 'Emergency Locksmith', description: 'إذا أغلقت سيارتك والمفتاح بالداخل، يتوفر لدينا فنيين معتمدين لفتح الأقفال بدون أي أضرار للمركبة.', arDescription: 'إذا أغلقت سيارتك والمفتاح بالداخل، يتوفر لدينا فنيين معتمدين لفتح الأقفال بدون أي أضرار للمركبة.', icon: 'key', basePrice: 150 },
          { id: 'mechanic', name: 'ميكانيك وكهرباء الطوارئ', arName: 'ميكانيك وكهرباء الطوارئ', enName: 'Emergency Mechanics', description: 'تشخيص الأعطال البسيطة وإصلاحها في موقعك (مثل السلف، الفيوزات، السيور) لتعود إلى الطريق بسرعة.', arDescription: 'تشخيص الأعطال البسيطة وإصلاحها في موقعك (مثل السلف، الفيوزات، السيور) لتعود إلى الطريق بسرعة.', icon: 'wrench', basePrice: 200 },
          { id: 'towing', name: 'سحب ونقل المركبات', arName: 'سحب ونقل المركبات', enName: 'Car Towing', description: 'رافعات متخصصة لنقل سيارتك بأمان تام إلى أقرب مركز صيانة أو إلى منزلك، متوفرة على مدار الساعة.', arDescription: 'رافعات متخصصة لنقل سيارتك بأمان تام إلى أقرب مركز صيانة أو إلى منزلك، متوفرة على مدار الساعة.', icon: 'truck', basePrice: 150 },
          { id: 'battery', name: 'شحن واستبدال البطارية', arName: 'شحن واستبدال البطارية', enName: 'Battery Jumpstart', description: 'تعطلت بطارية سيارتك فجأة؟ نوفر خدمة شحن البطارية السريع أو استبدالها ببطارية جديدة مكفولة في موقعك.', arDescription: 'تعطلت بطارية سيارتك فجأة؟ نوفر خدمة شحن البطارية السريع أو استبدالها ببطارية جديدة مكفولة في موقعك.', icon: 'zap', basePrice: 120 },
          { id: 'taxi', name: 'توصيل تكسي خاص و VIP', arName: 'توصيل تكسي خاص و VIP', enName: 'VIP Taxi Delivery', description: 'طلب سيارة أجرة مريحة أو مركبة عائلية واسعة بنظام الضمان المالي لحمايتك وضمان وصولك بسلام.', arDescription: 'طلب سيارة أجرة مريحة أو مركبة عائلية واسعة بنظام الضمان المالي لحمايتك وضمان وصولك بسلام.', icon: 'car', basePrice: 45 }
        ];
  ...
  // Synchronize registered users from Firestore (real-time)
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      console.warn("Users sync notice:", error);
    });
    return () => unsub();
  }, []);
...
معرف المهمة: #${reqId}`,
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased selection:bg-amber-500 selection:text-black pb-24 md:pb-0">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 select-none">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-14 sm:h-16 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => setActiveTab('home')}>
            <img src="/icon.svg" alt="Systro Logo" className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl shadow-sm object-contain shrink-0 border-2 border-sky-400/40 bg-slate-100 p-1" />
            <span className="text-sm sm:text-base font-black tracking-wider text-sky-600 font-mono">systro</span>
          </div>
          ...
          {activeTab === 'admin' && isAdminUnlocked && (
            <button 
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 border border-emerald-500/20 rounded-xl text-[9px] sm:text-[10px] font-black transition-all cursor-pointer shrink-0 animate-fade-in"
            >
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="relative p-2 sm:p-2.5 bg-white border border-slate-200 rounded-xl text-slate-600 hover:text-slate-950 hover:bg-slate-50 transition-all flex items-center justify-center cursor-pointer shrink-0"
              title={lang === 'ar' ? 'مركز الإشعارات الذكي (FCM)' : 'Smart Notification Hub (FCM)'}
            >
              <Bell className={`w-4 h-4 shrink-0 ${notifications.some(n => !n.isRead) ? 'animate-bounce text-amber-600' : ''}`} />
            ...
            <button 
              ...
              className="p-2 sm:p-2.5 bg-white border border-slate-200 rounded-xl text-slate-600 hover:text-slate-950 hover:bg-slate-50 transition-all text-xs font-bold flex items-center gap-1 sm:gap-1.5 cursor-pointer shrink-0"
            >
            ...
            {!isLoggedIn ? (
              <button 
                ...
                className="hidden lg:flex items-center gap-1.5 px-4 h-11 bg-white border border-slate-200 hover:border-slate-300 text-xs font-bold text-slate-700 rounded-xl transition-all cursor-pointer shadow-sm hover:bg-slate-50"
              >
                <span>→]</span>
              ...
              <div className="hidden lg:flex items-center gap-3">
                <div className="flex flex-col items-end text-right select-none">
                  <span className="text-xs font-black text-slate-800">{loggedInUserName}</span>
                ...
                <button 
                  onClick={async () => {
                    ...
                    if (newRole === 'technician') {
                      ...
                      if (cleanEmail) {
                        sessionStorage.setItem(`systro_tech_is_online_${cleanEmail}`, 'false');
                      } else {
                  ...
                  className="px-3.5 h-11 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 rounded-xl text-xs font-bold text-amber-600 transition-all flex items-center gap-1.5 cursor-pointer"
                ...
                <button
                  ...
                  className="px-3.5 h-11 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                ...
                >
                  {userAvatar ? (
                    ...
                    <User className="w-3.5 h-3.5 shrink-0 text-amber-600" />
                  )}
                ...
                <button
                  ...
                  className="px-3.5 h-11 bg-red-600/10 hover:bg-red-600/20 border border-red-500/20 rounded-xl text-xs font-bold text-red-500 transition-all flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95"
            ...
            {isLoggedIn && (
              <div className="lg:hidden flex flex-col gap-1 items-center justify-center shrink-0 my-0.5">
                ...
                <button
                  ...
                  className="p-1.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg text-slate-700 transition-all cursor-pointer flex items-center justify-center shadow-sm"
                  title={lang === 'ar' ? 'الملف الشخصي' : 'User Profile'}
                >
                  {userAvatar ? (
                    <img src={userAvatar} alt="avatar" className="w-4 h-4 rounded-full object-cover border border-amber-500/50" referrerPolicy="no-referrer" />
                ...
                <button
                  ...
                  className="p-1.5 bg-red-600/10 hover:bg-red-600/20 border border-red-500/20 rounded-lg text-red-500 transition-all cursor-pointer flex items-center justify-center shadow-sm active:scale-95"
      ...
      {allRequests.filter(isPendingForTechnician).length > 0 && (
        <div className="bg-amber-50 border-y border-amber-200 text-amber-900 px-4 py-2.5 shadow-sm flex items-center justify-between gap-3 text-xs font-bold select-none z-30 sticky top-14 sm:top-16 animate-fade-in">
          <div className="flex items-center gap-2 max-w-4xl truncate">
            ...
            <span className="truncate text-amber-950 text-[11px] font-black">
              {lang === 'ar' 
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 animate-fade-in space-y-8 bg-slate-50 text-slate-900 rounded-3xl border border-slate-200 shadow-2xl my-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-3xl p-5 max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-slate-800">
                <div className="space-y-1 text-center sm:text-right rtl:sm:text-right ltr:sm:text-left">
                  <h4 className="text-sm font-black text-red-700 flex items-center justify-center sm:justify-start gap-2">
                    <RotateCcw className="w-4 h-4 text-red-500" />
                    <span>{lang === 'ar' ? 'إعادة ضبط المحاكاة وقاعدة البيانات ⚠️' : 'Reset Database & Simulator ⚠️'}</span>
                  </h4>
                  <p className="text-xs text-slate-600 font-semibold">
                    {lang === 'ar' ? 'مسح جميع بيانات البلاغات، الدردشات، والمهمات المؤقتة وإرجاع النظام لحالته الأولية النظيفة.' : 'Clear all test alerts, chats, and emergency logs to restore clean default state.'}
                  </p>
                </div>
                <button
                  onClick={handleResetDatabase}
                  className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-xl shadow-md transition-all cursor-pointer shrink-0 flex items-center gap-2"
                >
                  <RotateCcw className="w-4 h-4" />
                  <span>{lang === 'ar' ? 'إعادة الضبط الآن' : 'Reset All Data'}</span>
                </button>
              </div>
              ...
              <div className="p-6 bg-white border border-slate-200 shadow-sm rounded-3xl space-y-6">
                ...
                {showAddServiceForm && (
                  <div className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-4 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right rtl:text-right ltr:text-left">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">
                          {lang === 'ar' ? 'أيقونة الخدمة (رمز تعبيري Emoji):' : 'Service Icon (Emoji):'}
                        </label>
                        <input 
                          type="text"
                          placeholder="e.g. 🛠️, 🛞, ⚡"
                    ...
                    <div className="flex flex-col gap-1.5 text-right rtl:text-right ltr:text-left">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">
                        {lang === 'ar' ? 'وصف تفصيلي للخدمة (بالعربية):' : 'Detailed Description (Arabic):'}
                      </label>
                      <textarea 
                        rows={2}
                        placeholder="اكتب وصفاً مختصراً يوضح ما تشمله هذه الخدمة للعميل..."
                        value={srvArDesc}
                        onChange={(e) => setSrvArDesc(e.target.value)}
                        className="w-full px-4 py-2 bg-white border border-slate-200 rounded-xl focus:border-amber-500 outline-none text-slate-800 text-xs font-medium"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2">
                      ...
                      <button
                        ...
                        className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-xl shadow-md transition-all cursor-pointer flex items-center gap-2"
                      >
                        <Plus className="w-4 h-4" />
                        <span>
                          {editingService 
                            ? (lang === 'ar' ? 'حفظ تعديلات الخدمة 💾' : 'Save Service Changes 💾') 
                            : (lang === 'ar' ? 'إضافة الخدمة الجديدة للشبكة 🚀' : 'Add New Service to Network 🚀')}
                        </span>
                      </button>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {dbServices.map((srv) => (
                    <div key={srv.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 relative text-right rtl:text-right ltr:text-left">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                        <span className="text-xl">{srv.icon || '🛠️'}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-0.5 rounded-full font-mono">
                            {srv.basePrice} ₪
                          </span>
                        </div>
                      </div>
                      <div>
                        <h4 className="text-xs font-black text-slate-900">{srv.arName || srv.name}</h4>
                        <p className="text-[10px] text-slate-500 font-semibold line-clamp-2 mt-1">{srv.arDescription || srv.description || (lang === 'ar' ? 'خدمة إنقاذ ومساعدة سريعة معتمدة بالشبكة.' : 'Certified emergency service.')}</p>
                      </div>
                      <div className="flex items-center justify-end gap-2 pt-1 border-t border-slate-200">
                        <button
                          onClick={() => handleEditService(srv)}
                          className="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-slate-700 text-[10px] font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1"
                        >
                        ...
                        <button
                          ...
                          className="px-3 py-1 bg-red-100 hover:bg-red-200 text-red-700 text-[10px] font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1"
                        >
                ...
                {/* Pending Custom Service Proposals */}
                {pendingServices && pendingServices.length > 0 && (
                  <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl space-y-3">
                    <h4 className="text-xs font-black text-amber-700 flex items-center gap-2">
                      <PlusCircle className="w-4 h-4 text-amber-500" />
                      <span>{lang === 'ar' ? 'اقتراحات الخدمات الجديدة بانتظار الموافقة' : 'Pending Custom Service Proposals'}</span>
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {pendingServices.map((srv) => (
                        <div key={srv.id} className="p-3 bg-white border border-slate-200 rounded-xl space-y-2 text-right">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-black text-slate-800">{srv.arName || srv.name}</span>
                            <span className="text-[10px] text-slate-500 font-mono">{(srv as any).estimatedPrice ? `${(srv as any).estimatedPrice} ₪` : 'حسب العرض'}</span>
                          </div>
                          <p className="text-[10px] text-slate-600">{srv.arDescription || srv.description}</p>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-[60] bg-white/95 backdrop-blur-xl border-t border-slate-200 px-2 py-2 flex items-center justify-around text-center shadow-lg select-none">
        <button
          onClick={() => setActiveTab('home')}
          className={`flex flex-col items-center gap-0.5 transition-all ${activeTab === 'home' ? 'text-amber-600 font-black scale-105' : 'text-slate-500 font-bold hover:text-slate-800'}`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 transition-all ${activeTab === 'client' ? 'text-amber-600 font-black scale-105' : 'text-slate-500 font-bold hover:text-slate-800'}`}
        >
          <User className="w-5 h-5" />
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 transition-all relative ${activeTab === 'simulator' ? 'text-amber-600 font-black scale-105' : 'text-slate-500 font-bold hover:text-slate-800'}`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 transition-all ${activeTab === 'world' || isLeftMenuOpen ? 'text-amber-600 font-black scale-105' : 'text-slate-500 font-bold hover:text-slate-800'}`}
        >
        ...
        <button
          onClick={() => setActiveTab('store')}
          className={`flex flex-col items-center gap-0.5 transition-all ${activeTab === 'store' ? 'text-amber-600 font-black scale-105' : 'text-slate-500 font-bold hover:text-slate-800'}`}
        >
